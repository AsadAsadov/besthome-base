...
class ParamSyncApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            control_frame, text="📆 Son günlər (-10 və s.):", text_color=TEXT
        ).grid(row=0, column=4, padx=(10, 6), pady=12, sticky="w")
        ...
        ctk.CTkLabel(
            control_frame, text="🗂️ Verilənlər bazası mənbəyi:", text_color=TEXT
        ...
        self.db_source_entry.grid(
            row=1, column=1, columnspan=4, padx=(0, 12), pady=(0, 12), sticky="we"
        )
        ...
        self.db_source_button.grid(
            row=1, column=5, padx=(0, 12), pady=(0, 12), sticky="e"
        )
        ...
        self.progress_bar = ctk.CTkProgressBar(
            main_frame, height=14, progress_color=PRIMARY
        )
    ...
    def choose_db_source(self):
        ...
        if file_path:
            ...
            self.progress_label.configure(
                text=f"📁 Seçilmiş mənbə: {file_path}", text_color="#555"
            )
        else:
    ...
    def update_statistics(self):
        try:
            ...
            self.detail_label.configure(
                text=f"⚠️ Statistika xətası: {err}", text_color="#E74C3C"
            )
        ...
        def worker():
            try:
                ...
                self.sync_button.configure(
                    text="🔄 Yüklənir...", width=self.sync_button_width
                )
                ...
                self.progress_label.configure(
                    text=f"❌ Xəta: {err}", text_color="#E74C3C"
                )
            finally:
                time.sleep(0.1)
    ...
    def pause_sync(self):
        if self.sync_controller.state == SyncStateController.RUNNING:
            ...
            self.progress_label.configure(
                text="⏸️ Sinxronizasiya dayandırıldı", text_color="#E67E22"
            )
            self._update_state_label()
    ...
    def resume_sync(self):
        if self.sync_controller.state == SyncStateController.PAUSED:
            ...
            self.resume_button.configure(state="disabled")
            self.progress_label.configure(
                text="▶️ Sinxronizasiya davam edir", text_color="#0078D4"
            )
            self._update_state_label()
    ...
    def _update_state_label(self):
        ...
        if state == SyncStateController.RUNNING:
            ...
            self.state_label.configure(
                text="Status: Dayandırılıb", text_color="#E67E22"
            )
        else:
